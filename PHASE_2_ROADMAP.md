...
- [ ] Set up CI (GitHub Actions): `typecheck` + `npm test` on every push, so a broken commit can't sit unnoticed
  - ✅ Test-data decision made (2026-07-19): **cleanup script against the live project** — so CI runs `npm test` against live Supabase and the script purges afterward.
- [ ] Establish a test-data cleanup routine (**DECIDED 2026-07-19: purge/cleanup script, not a separate test project**): Phase 1 test runs have been seeding accounts/clients into the production database. Build the script on **Workstream D's** deactivation + PII-scrub path rather than as a parallel one-off.
...
- [x] **Test-data strategy** (unblocks CI in Workstream 0) — **DECIDED 2026-07-19: cleanup script against the live Supabase project** (not a separate test project). Interlocks with **Workstream D** — the deactivation + PII-scrub path is the natural mechanism for the e2e suite's cleanup, so build the script on top of it rather than as a parallel one-off.
