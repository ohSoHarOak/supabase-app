...
- [ ] **Security review #1 — before any Phase 2 feature work (pairs with Workstream 0):**
  - Tenant-isolation audit: the server talks to Supabase with the **service key** and there are **no Postgres RLS policies** (verified 2026-07-15) — every bit of data isolation lives in app code scoping queries by `professional_account_id`. Audit every endpoint for a missing scope, and add RLS as a second net (**DECIDED 2026-07-19: approved** — the owner portal is live, so the second account type reading the same tables is now real; scoped as migration work here)
