# Sit.Stay.Play — Phase 4 Roadmap: App Design & Brand Identity
...
1. **Brand identity** — the Sit.Stay.Play mark, wordmark, and lockups; the palette promoted from "colors that were picked in Week 4" to a defined system; typography; iconography; voice.
...
- [ ] ⏩ **B-3: Design the mark.** The paw is the obvious route and also the most crowded one — worth exploring at least one non-paw direction (a leash line, a tag silhouette, a monogram) before defaulting. Requirements, all non-negotiable, from where the mark actually has to live:
  - Legible at **16px** (favicon) and inside a **maskable safe zone** (Android adaptive icons crop to a circle, squircle, or rounded square depending on launcher — art must sit inside the center 66%).
...
- [~] ⏩ **B-8: ⚠️ Trademark sanity check before the mark is finalized.** "Sit.Stay.Play" is a common construction in this market. A quick USPTO + app-store search *before* the logo is on a Play listing, business cards, and every client's invoice is cheap; discovering a conflict afterward is not. Founder task, possibly with counsel — the same counsel who reviewed the contract template.
  - **Finding 2026-09-04 (founder search): there is an existing "Sit.Stay.Play" trademark, status DEAD.** Recorded so the next naming conversation doesn't start by re-running the same search.
  - **What DEAD does and doesn't mean** — worth writing down, because it cuts both ways and the intuitive read is the wrong one. Dead means abandoned, cancelled, or expired, and a dead mark is **generally available to register**, so on its own it does *not* disqualify "Sit.Stay.Play." What it *does* establish is that someone was operating under this name in this space, which matters for two reasons registration status doesn't capture: **common-law rights can outlive a lapsed registration**, and a dead mark can sometimes be **revived**. That gap between "the register says dead" and "nobody has rights" is the part worth taking to counsel rather than settling from a search result.
  - ⚠️ **Still `[~]`, not `[x]`.** One search on the incumbent name is not the clearance this item asks for. It has to cover whatever name actually ships, and the app-store half hasn't been run at all.
  - **Sequencing confirmed in practice:** this file previously treated B-8 as something to do before finalizing the mark. 2026-09-04 showed it belongs **before D4-6** as well — the search result is what reopened the naming question, so deciding the name first would have meant deciding it twice.
...
- [ ] **V-7: Print stylesheet.** A real `@media print` block for the signed contract (W-1) and the printable invoice/receipt (P2-7/F-6): no chrome, no nav, black-on-white, the one-color mark, page-break rules, URLs spelled out. Coordinate with P2-7 so professional branding and Sit.Stay.Play branding don't fight on the same page.
...
- [ ] **A-6: Scheduling + walk completion.** Includes the walk report card — which per `SPEC.md` is shared by owners with light Sit.Stay.Play branding, making it the product's organic acquisition surface. It deserves the most design attention of any single component.
...
- [ ] **E-2: Printable documents.** Signed contract (W-1) and printable invoice/receipt (F-6/C-4/R-12, spec'd under P2-7 in Phase 3), on V-7's print stylesheet. ⚠️ **Coordinate with P2-7:** the walker's logo is the primary brand on their invoice; Sit.Stay.Play's is a footer credit at most. Get that hierarchy right or it looks like the walker is billing on someone else's stationery.
- [ ] **E-3: Shared walk report card.** `SPEC.md` designates this the organic acquisition channel — an owner shares a walk card and a stranger sees Sit.Stay.Play. Design for the share context (screenshot, text message, social crop), including the API-free SVG route trace `SPEC.md` specifies. **Depends on P2-3 (photos) and P2-2 (GPS trace); design it now so those land into a finished shape.**
- [x] **E-4: Marketing/landing page.** — **DONE 2026-09-06, ahead of Phase 4 and for a reason nobody planned:** Stripe's business-URL review wants a page describing what the business does, and a login screen does not qualify. Built as a standalone static site (`landing/index.html`, own Render service, no build step) rather than taking over the app's `/`, so it cannot affect the PWA `start_url`, the service worker's navigation caching, or existing bookmarks. Live at **https://sitstayplay.pro** (verified 200 over HTTPS, byte-identical to source). Content is limited to what actually exists — no invented metrics or testimonials — and carries no prices, since tiers are decided but unpriced. Still a Phase 4 candidate for a *designed* replacement once M-Brand lands; this is a working page, not a finished brand expression.
...
- [x] **D4-4: Pull the logo + Android icon forward into Workstream M?** — **DECIDED 2026-08-23 (founder): yes.** Scheduled as **M-Brand** in `PHASE_2_ROADMAP.md`; scope is **B-1…B-4, B-8 + N-1…N-3** (N-3 and B-8 added to the original recommendation — see "Pull This Forward" for why). Specs stay in this file; the schedule lives in Phase 2. **Two consequences accepted with the decision:** the mark is drawn before typography (B-5) is chosen, so the **wordmark/lockups may need a second pass** later — the symbol itself, which is what the icon uses, is unaffected; and **B-2 needs D4-6 answered** ("Sit.Stay.Play" vs "Sit.Stay.Play") before the wordmark can be drawn, which promotes D4-6 from a someday-decision to a near-term one.
- [x] ~~**D4-5: Is a public marketing/landing page in scope (E-4), or does the Play listing carry it?**~~ — **ANSWERED 2026-09-06 in practice rather than by decision:** Stripe forced it. A landing page now exists and is live, so the Play listing does not carry that load alone. ⚠️ Recorded honestly: this was not a considered brand decision, it was an unblock. If Phase 4 wants a designed marketing site, the existing page is a starting point to replace, not evidence the question was settled on merit.
- [ ] **D4-6: "Sit.Stay.Play" vs "Sit.Stay.Play"** — which is the brand, which is the product (B-2)? Also worth settling how this relates to the sending domains already in use (`eastwestoak.com` verified at Resend, `itchytail.com` also verified) — a brand mismatch in the From line costs deliverability trust. **Still OPEN**, but no longer uninformed — see the B-8 finding above (a dead "Sit.Stay.Play" registration exists).
  - **Rejected 2026-09-04: `Walkable`.** Proposed and withdrawn the same day. Two reasons, both worth keeping so it isn't re-proposed: it **names the single service the product is architected to grow past** (trainers, groomers, sitters, boarding — `CLAUDE.md` hard constraints, Workstream X), and it would have shipped **ahead of its own B-8 clearance**, which is the exact ordering mistake the finding above just demonstrated. A dictionary word in a crowded adjacent space ("walkability" scoring, urban-planning products) is also a harder clearance than a coined name, not an easier one.
  - 📌 **Implementation cost is not a reason to delay this decision.** The full mechanical rename was executed and reverted on 2026-09-04 as a working spike: ~20 files across both frontends, the emails, the manifest, the Android label and the service worker, with `typecheck` / `check:structure` / `build:web` / **e2e 16/16** all green afterward. It takes minutes for any name. What is *not* cheap is everything downstream of a shipped name — `appId`, the Play listing, client-facing emails, invoices — so the constraint on this decision is clearance and store timing, never the code.
  - ⚠️ **`appId` rides on this and hardens permanently.** `capacitor.config.ts` pins `com.petpro.connect`, mirrored in `android/.../strings.xml` as `package_name` and `custom_url_scheme`. It is invisible to users and harmless while it disagrees with a new brand — but it **cannot be changed after the first Play upload**. Whatever D4-6 concludes, settle appId in the same sitting.
