...
- [~] ⏩ **B-8: ⚠️ Trademark sanity check before the mark is finalized.** "PetPro" is a common construction in this market. A quick USPTO + app-store search *before* the logo is on a Play listing, business cards, and every client's invoice is cheap; discovering a conflict afterward is not. Founder task, possibly with counsel — the same counsel who reviewed the contract template.
  - **Finding 2026-09-04 (founder search): there is an existing "PetPro Connect" trademark, status DEAD.** Recorded so the next naming conversation doesn't start by re-running the same search.
  - **What DEAD does and doesn't mean** — worth writing down, because it cuts both ways and the intuitive read is the wrong one. Dead means abandoned, cancelled, or expired, and a dead mark is **generally available to register**, so on its own it does *not* disqualify "PetPro Connect." What it *does* establish is that someone was operating under this name in this space, which matters for two reasons registration status doesn't capture: **common-law rights can outlive a lapsed registration**, and a dead mark can sometimes be **revived**. That gap between "the register says dead" and "nobody has rights" is the part worth taking to counsel rather than settling from a search result.
...
- [ ] **D4-6: "PetPro" vs "PetPro Connect"** — which is the brand, which is the product (B-2)? Also worth settling how this relates to the sending domains already in use (`eastwestoak.com` verified at Resend, `itchytail.com` also verified) — a brand mismatch in the From line costs deliverability trust. **Still OPEN**, but no longer uninformed — see the B-8 finding above (a dead "PetPro Connect" registration exists).
