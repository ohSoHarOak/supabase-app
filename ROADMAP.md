...
3. **Email visibility.** ✅ **The sending domain is `eastwestoak.com`, verified at Resend 2026-07-17** (not `itchytail.com` — that was the earlier plan and is superseded; older entries below still name it). ⚠️ **But Render's `EMAIL_FROM` still says `info@itchytail.com`, so sends are refused until it's changed** — see the blocker under the O-2 note in Running Notes. Until that's fixed, notification email reaches nobody; a "+" alias of your own inbox (e.g. `nitro.shae.clark+pup@gmail.com`) is still the safest demo client either way.
