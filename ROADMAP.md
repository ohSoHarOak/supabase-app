...
- [ ] **W-5: service form moves out of client setup and into contract generation.** Field order: **Pet → Type → Price → # of sessions → Billed → Duration → Notes** (multi-line). "Name" is removed from the form.
  - *Decisions:* Name is **auto-built** as "Type — Pet" (e.g. "Private walk — Pepper") and written to the existing NOT NULL `services.name`, so **no migration** and invoice descriptions (`SchedulingService.ts:525`) keep reading naturally. Notes **reuses the unused `services.description` column** — also no migration.
- [ ] **W-6: one contract carries multiple services, each with its own pet selection.** Repeatable service block with "＋ Add another service", so Pepper can be on private walks while Biscuit is on a training package.
  - *Decision:* **multi-pet per service** via the `service_pets` join table — it exists in the schema but **has never been written to** (dead table since Week 2), so this is wiring, not a migration. One walk covering two dogs = one service, one price, one auto-invoice.
  - *Fixes a live assumption:* today `{{pet_list}}` is auto-filled with **every** pet on the client and there is no selection anywhere. A contract's pet coverage becomes the union of the pets across its services.
- [ ] **W-7: services are born on signing, and live on the client profile.** On **signed** (not generated — a draft can still be edited or voided), the contract's services appear on the client profile.
  - *Decision:* profile services are **contract-fed but editable** (incl. Notes) — no ad-hoc "add" button, so services can't drift from what the contract promised.
- [ ] **W-8: "＋ Add services" on the client profile reuses the contract flow.** Opens contract generation scoped to just the new services → client signs → they attach. Deliberately **not** a bypass: an unsigned add-service button quietly reintroduces the problem W-5…W-7 exist to fix.
- [ ] **W-9: contract template — services table + addendum variant. ⚠️ Needs counsel review.** Key Terms currently has fixed `Walk type` / `Schedule` / `Fees` rows; multi-service (W-6) turns those into a **services table**. W-8 additionally wants an **addendum** variant ("supplements the agreement dated X") rather than a fresh standalone agreement. Both change the part of the document that actually binds — route through the same legal review already flagged for the CA template's two substantive edits.
...
**⚠️ Sequencing — founder call outstanding.** This is a flow change to the spine of the demo (contract → services → auto-invoice), it re-opens the QA pass closed 2026-07-16 (weeks 3, 5, 6 all touch services), and W-9 has a **legal-review dependency that isn't in our control**. The build was green and demo-ready as of 2026-07-16. Decide explicitly: ship the demo on the current build and do W-5…W-13 as Phase 1.5, or hold the demo and take the QA + counsel round-trip.
...
| 8 — Owner Portal + Demo | 🔄 QA pass 2026-07-16: weeks 1–7 + `npm test` all green, payment loop closed live (webhook verified). Q-1 fixed (needs deploy). Open: Q-2 Supabase allowlist, Q-4 declined card | 🔄 Supabase settings, sign-off, declined-card run pending |
