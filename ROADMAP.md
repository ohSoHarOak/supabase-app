...
- [ ] **T-1: separate load-bearing hooks from cosmetic classes.** Anything the JS queries or that carries layout-critical sizing moves to a `data-*` attribute (`data-doc-shell`, `data-zoom-frame`); everything left in `class=` is purely cosmetic and belongs to the designers. This is the actual fix — it makes the two categories visually distinct at the edit site, so "can I change this?" has an obvious answer. Do this **before** the design handoff.
- [ ] **T-2: a structural contract test in `npm test`.** A small script asserting every load-bearing hook still exists in both frontends and that the sign pane resolves to a non-zero height in both. Catches the *next* divergence instead of the last one. Cheap (~20 lines), and it's the safety net that makes a CSS rewrite survivable.
...
- **Hand the design team a written list of load-bearing selectors** as part of the handoff. After T-1 that list is just "every `data-*` hook", which is the point.
