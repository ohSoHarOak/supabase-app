...
- [x] Decide: for weekly/monthly-billed services, when should the invoice generate? (End of the billing week/month, or with the first completed walk of the period?) Per-visit auto-invoicing works today; this decision unlocks the other cadences.
  - Deferred 2026-07-15 at week close; **decided by founder 2026-07-17: end of the billing period** — and built + deployed same day. Migration 017 adds `services.next_invoice_date`; a worker (boot + every 5 min) invoices due weekly/biweekly/monthly services and advances the date, exactly-once per period. The clock starts at creation/signing, re-anchors on W-13 resume (paused time is never billed), and stops past the W-13 end date. Pre-existing services are scheduled forward, never back-billed. Verified live: backdated weekly period → exactly one "week ending …" invoice, second pass generated nothing.
- [x] Confirm status: mark this week done, or note what broke
...
- [x] **W-9: CLOSED by founder decision 2026-07-17 — counsel packet removed; the Pet Services Agreement is the only seeded template.** The CA v2 + addendum drafts and `COUNSEL_REVIEW.md` were deleted from the repo; the CA v1 file is retained unseeded (accounts that copied it keep their copy; signed contracts are immutable snapshots regardless), and the new-contract form defaults to the Pet Services Agreement. The addendum concept is retired with the packet — adding services to an existing client generates a fresh Pet Services Agreement. **The legal-review recommendation from Week 3 stands as the standing caveat:** the in-document notice tells both parties to have their own counsel review before signing, and no counsel has reviewed this template.
  ...
  - ~~Drafts ready to send~~ **Superseded 2026-07-17: the packet and both drafts were removed at founder direction** (they existed as of commit `2ff1213` if ever needed again). History of what the packet asked is preserved in the bullets below.
...
## Cold-Tester Feedback — F-1…F-9 (logged & built 2026-07-18)

> Feedback from the founder's recruited tester (the Week 8 "test the portal cold" task), in two rounds the same day. F-1…F-5 and F-7…F-9 built and browser-verified 2026-07-18; F-6 was a question, answered below.

- [x] **F-1: "Add a pet" form is distracting under the pet list — minimize it.** Once a client has at least one pet, the form collapses behind a "＋ Add another pet" button (opens the form and focuses it). Still open by default when there are no pets yet, and during step 2 of new-client setup.
- [x] **F-2: "New invoice" should be minimized once an invoice exists.** Same pattern: the Billing section shows the invoice list, with the form behind "＋ New invoice" once the first invoice exists.
- [x] **F-3: schedule services from the client profile.** A "📅 Schedule" button now sits in the client-profile header (next to Message / Generate contract), and each active service row already had its own Schedule button. Bonus: the signed-contract view now has "📅 Schedule walks", so sign → schedule is one click.
- [x] **F-4: after signing, scheduling asked for a Service name etc.** Root cause: the booking form still carried a Week-6-era inline "＋ New service…" block — the same contract-bypass drift W-8 removed from the client profile, surviving in one more place. Removed. The service dropdown pre-fills from the client's contract-born active services (name, price, duration all derived from what the client signed); a client with no active service gets pointed at contract generation (submit disabled + "Generate contract" shortcut) instead of a blank form. `POST /api/services` still exists for scripts/seeds — as with W-8, the UI bypass was the problem, not the endpoint.
- [x] **F-5: can't schedule multiple days per week of a service.** Built, backend + UI. Booking a Weekly appointment now shows Mon–Sun checkboxes ("On which days?"); the first walk's weekday is locked in, extra days repeat weekly at the same time. API: `extra_starts` on `POST /api/appointments` — the whole multi-day set is conflict-checked (including against itself) and books as **one** series (one parent, children linked), so "End series" ends all days at once and a clash 409s the entire request, nothing half-lands. Verified: Mon/Wed/Fri × 3 weeks = 9 walks in-browser; `npm test` step 5 extended with a multi-day leg (2 days × 2 weeks = 4 rows, correct dates, series-cancel ends all 4). Weekly "for how many weeks" minimum also relaxed 2 → 1 (e.g. 3 walks this week only).
- [x] **F-7: clients on the same day should show as different colors on the schedule (logged & built 2026-07-18, second feedback round).** Every client gets a stable color: avatar in the Today/Clients/Messages lists, 4px left stripe on every calendar card (Schedule week view + Today's agenda). *Design decision worth knowing:* colors are assigned by **client creation order**, not a hash of the id — an 8-color hash collided between the first two test clients immediately, which defeats the feature on the exact screen it's for. Creation order guarantees consecutive clients differ and never reshuffles existing clients' colors when a new one is added (a 9th client wraps the palette). Verified in-browser: two clients booked the same Monday render steel vs sage, matching their list avatars.
- [x] **F-8: contract should fit the window, with zoom; signature hint overlapped by the pad (second round).** Three fixes on the sign/view screen: (1) the document pane now fills the window height (`clamp(420px … 100vh-310px … 900px)`) instead of a fixed 440px; (2) **Zoom − / % / ＋ controls** (50–180%) — implemented by scaling the sandboxed iframe with width compensation, so the text *reflows* to the visible width at every zoom level rather than growing a horizontal scrollbar; (3) the "Sign above with a finger or mouse" hint had a negative top margin pulling it under the pad — now sits 6px clear below it. Both the signable and the signed views get the zoom tools.
- [x] **F-9: phone/email shown in hard-to-read blue (second round).** The `tel:`/`mailto:` links on client detail had no color rule, so they rendered browser-default link blue. Now they inherit the ink text color with a soft underline (the underline alone marks them tappable) — readable in both themes.
- [x] **F-6 (question): where does the owner see all invoices, generated and paid?** Answered, no build needed: every invoice — open, paid, void — lives permanently in the **Billing** section of the client profile (professional side), and the client sees their own in the **owner portal** (unpaid ones as "Pay now" cues, paid ones under "Payment history"). Clients do **not** need a Stripe account — Stripe is only the hosted payment page, and Stripe e-mails a receipt on payment. A *clickable invoice document* (printable, branded copy per invoice) doesn't exist yet — that's **P2-7** (branded invoices); noted there.
...
3. **Email visibility.** ✅ **Resolved 2026-07-18 — email sends for real.** Domain `eastwestoak.com` verified at Resend, `EMAIL_FROM` on Render set to `info@eastwestoak.com`, confirmed by a live test send. (Not `itchytail.com` — that was the earlier plan; older entries below still name it.) For the demo, a "+" alias of your own inbox (e.g. `nitro.shae.clark+pup@gmail.com`) is still the most convenient client, since you can show the email arriving on screen.
...
The only way to get a magic link is for the owner to visit `/portal` and request one themselves (`POST /api/portal/login`). Nothing in the product ever tells them the portal exists — the walker has to say so out loud. The owner `account` row is then created automatically on first link click (`PortalService.ts:101`) and every client row with a matching email is linked, so **there is no "pet owner profile" to create by hand** — it materialises on first login. That part is fine; the discovery path is what's missing.
- Build: a `portal_invite` template (welcome + what they can do + a link to `/portal`), sent when a client with an email is created, and re-sendable from the client profile.
...
> Raised by the F-8 portal regression: `public/app.js` (professional, ~2,470 lines) and `public/portal.js` (owner, ~640 lines) are **fully independent frontends that share one `styles.css`**. There is no component layer, so a CSS change that adds or renames a *structural* element silently breaks whichever frontend didn't get the matching markup edit. F-8 moved the contract pane's height onto a new `.doc-shell` wrapper, updated only `app.js`, and the portal's iframe collapsed from 440px to **150px** — measured, not estimated. It shipped in `d8259b9` and no one saw it because the portal has never been cold-tested (see the note under F-1…F-9).

**This matters more now that a UX/UI update is coming.** The app currently depends on CSS contracts that are invisible to a designer: `.doc-shell` isn't decoration, it's load-bearing — delete its `height` and the sign screen breaks with no error, no failing test, and no console warning. A design pass over `styles.css` is exactly the change most likely to hit this, and least likely to notice.

**Goal: the design team can rewrite `styles.css` freely without breaking the app.** Three tiers, in dependency order.

- [x] **T-1: separate load-bearing hooks from cosmetic classes.** *(built 2026-07-18)* Anything the JS queries or that carries layout-critical sizing moves to a `data-*` attribute (`data-doc-shell`, `data-zoom-frame`); everything left in `class=` is purely cosmetic and belongs to the designers. This is the actual fix — it makes the two categories visually distinct at the edit site, so "can I change this?" has an obvious answer. Do this **before** the design handoff.
- [x] **T-2: a structural contract test in `npm test`.** *(built 2026-07-18)* A small script asserting every load-bearing hook still exists in both frontends and that the sign pane resolves to a non-zero height in both. Catches the *next* divergence instead of the last one. Cheap (~20 lines), and it's the safety net that makes a CSS rewrite survivable.
- [x] **T-3: one shared markup module for duplicated screens.** *(built 2026-07-18)* ⏭️ **Was sequenced first** (decision D3, 2026-07-18) — it blocks owner-facing self-service, because the portal profile work would otherwise copy `fmtPhone` and the contact-card markup into `portal.js` and recreate the F-8 divergence. See the Onboarding section's build order. `public/shared.js` exporting the sign pane (and later the message list and cards) as functions returning markup strings, loaded by both pages with a plain `<script>` — **no build step**, which keeps the current no-build architecture intact. Removes the duplication that made T-1 necessary in two places.
- ⚠️ **Deliberately not doing: merging the two frontends into one role-routed app.** Correct long-term, wrong now — it's a multi-day refactor of demo-critical code. Revisit in Phase 2 if the portal grows.
- **Hand the design team a written list of load-bearing selectors** as part of the handoff. After T-1 that list is just "every `data-*` hook", which is the point. ✅ *Superseded by the STRUCTURAL CONTRACT block at the top of `styles.css` — it is the list, it explains what breaks and why in plain language, and `npm run check:structure` enforces it. The handoff instruction is now one sentence: **"restyle anything below the contract block; if you need to change something inside it, talk to us and run `npm run check:structure`."***

---

## Cold-Test Round 2 — Portal Walkthrough (C-1…C-6, logged 2026-07-18)

> The **first real cold test of `/portal`** — the earlier F-1…F-9 round was run against the professional app by mistake (see the note under Frontend Architecture). Demo is on hold pending this work. Every finding below was traced to source before triage.

- [ ] **C-1: the signup/login email comes from Supabase, unbranded.** Correct, and it is a **dashboard fix, not code** — magic links are sent by Supabase's built-in mailer (not Resend), so the template lives in **Supabase → Authentication → Email Templates**. ⚠️ Same mailer is rate-limited to ~2 emails/hour (see the demo-day note) — don't test it in a loop.
- [x] **C-2: portal nav is only Home / Messages / Log out — no profile.** *(fixed 2026-07-18, `95a64d2` — read-only Profile tab)* Confirmed at `portal.js:109-121`. The deeper problem is the API, not the nav: **every portal route is read, sign, pay, or message** (`portal.ts` — no PATCH/PUT anywhere). Owner self-service is missing surface area, not a missing link. **Decision locked (D2): the portal profile is READ-ONLY.** Owners see their details and message their walker to correct them — no owner write path is being built. Rationale: the `clients` row is owned by `professional_account_id`; letting owners write to a walker's CRM record needs a conflict/approval story that isn't worth building for Phase 1.
- [x] **C-3: the walker's own information isn't shown to owners.** *(fixed 2026-07-18, `d032588` — see PH-1)* Root cause is upstream of the portal: **`accounts.phone` exists in the schema (`001_create_accounts.sql:15`) and in `types/index.ts:19`, but has no UI anywhere.** The Profile tab (`app.js:2070-2091`) collects only full name, business name, and services offered. The portal can't display the walker's number because the walker has never been able to enter it. Fix is two parts: add phone to the walker Profile (and to onboarding step 1), then render a walker contact card in the portal.
- [ ] **C-4: paid invoices aren't clickable.** Confirmed — `portal.js:217-223` renders paid invoices as static rows; only open ones get a "Pay now" button. Already scoped as **P2-7** (branded invoices); F-6 predicted this exact complaint. No new item, but this is now the second independent request for it.
- [ ] **C-5: no payment confirmation email or receipt. NOT a Stripe bug and NOT a code bug.** `PaymentService.ts:504-515` correctly enqueues both `payment_receipt` (to the client) and `payment_received` (to the professional) on the paid-exactly-once transition. Two independent real causes, both environmental:
  1. **Resend domain unverified** — notification email reaches only the Resend account owner's inbox. `npm test` step 10 has been reporting this the whole time.
  2. **Stripe test mode does not email customers receipts by default** — this is a Dashboard toggle (Settings → Customer emails), no code involved.
  - ⚠️ **`portal.js:459` currently tells the user "A receipt is on its way to your email."** Until (1) and (2) are resolved, the app is making a promise it cannot keep. Either fix the delivery or soften the copy.
  - Diagnostic: `select status, error, sent_at, payload->>'template' from notification_queue where category = 'payment' order by created_at desc limit 10;` — `pending` means no API key, `failed` + error means a domain/recipient problem.
- [x] **C-6: no signup / password creation for owners.** *(fixed 2026-07-18, `95a64d2` — copy moved above the email field)* **Working as designed** — `CLAUDE.md` locks *magic-link for owners, email/password for professionals*, and the owner account is auto-created on first link click (`PortalService.ts:101`). **Decision locked (D1): stay passwordless; fix the messaging.** The tester went looking for a signup button because nothing told them they didn't need one. The login screen must say so before the email field — passwordless is the right call for someone who logs in a handful of times a year, but only if it's stated.

### Phone-format audit (requested 2026-07-18) — 4 consistent, 3 gaps

`fmtPhone` (`app.js:129`) normalises US 10/11-digit input to `(555)010-1234`, leaves international/extensions untouched, and applies via a delegated `focusout` on `data-phone` plus again on save.

| Field | Formatted? |
| --- | --- |
| `c-phone`, `c-ecphone` (new client) | ✅ |
| `ec-phone`, `ec-ecphone` (edit client) | ✅ |
| Client-detail display (`:791`, `:794`) | ✅ |
| **`accounts.phone`** — the walker's own | ❌ **no UI exists at all** (drives C-3) |
| **`pets.emergency_vet`** (`:850`, `:956`) | ❌ free text, `"Clinic, phone"` packed into one column |
| **`portal.js`** | ❌ no phone handling anywhere |

- [x] **PH-1: add `accounts.phone` to the walker Profile tab + onboarding step 1**, with `data-phone`. Unblocks C-3. *(built 2026-07-18, `d032588`)* — no migration needed; the column has existed since 001. `PATCH /api/auth/profile` now accepts `phone` and **splits it off** to `AccountService.updateAccountPhone` rather than duplicating the column onto `professional_profiles`. Onboarding step 1 needs no backend work: **`POST /api/auth/signup` has always accepted `phone`** (`auth.ts:16`) — only the Profile tab lacked it.
- [ ] **PH-2: split `pets.emergency_vet` into clinic name + phone.** Today it's one free-text string, so it can't be formatted, can't be searched, and can't become a `tel:` link — on the one screen where someone may need to call a vet urgently. Needs a migration + a backfill that splits existing values on the first digit run.
- [ ] **PH-3: when the portal gains any phone display, `fmtPhone` must be shared, not copied.** ⚠️ Copy-pasting it into `portal.js` recreates the exact duplication that caused the F-8 regression — see the T-3 sequencing note below.

---

## Onboarding — Walker & Pet Owner (mapped 2026-07-18, decisions locked)

> Requested after the C-1…C-6 walkthrough. **Sequencing decision (D3): T-3 (shared markup module) lands BEFORE any owner-facing self-service**, so the second frontend doesn't grow a second copy of everything first.

**Reuse, not new invention:** `renderNewClient` (`app.js:605-615`) already implements a 3-step `.stepper` with "you can stop after any of them and finish later" semantics. Both flows below copy that pattern — same markup, same CSS, and returning users have already seen it once. **No external library is a fit here** — the no-build vanilla-JS architecture rules out the React-based options, and the tour libraries (driver.js, Shepherd, intro.js) solve tooltips-over-existing-UI, which is not this problem.

### O-1: Walker onboarding — 3-step wizard, immediately after signup ✅ *(built 2026-07-18, `741dfe2`)*
**Built as `#/setup` (steps at `#/setup/2`, `#/setup/3`).** "Done" is **derived** from the profile — `full_name` + `phone` + at least one offered type — rather than a flag column, so there was nothing to migrate and nothing to keep in sync. Reuses the existing `.stepper` markup and `PATCH /api/auth/profile`: no new endpoint, no schema change, no new CSS. Today carries a "Finish setting up" cue while incomplete; the app is never blocked and every step is skippable.
⚠️ **Scope note — step 2's default price/duration per service type was NOT built.** Nothing in the schema stores it (services are contract-born by design, per F-4), so it needs a migration and a new concept. Logged as **P2-14** rather than smuggled into this commit.
Today signup drops the walker into an empty app with no guidance. `business_name` is optional and phone doesn't exist, so **a contract can generate with a blank business name** and booking can fail for want of a service. Each step below is a precondition something currently allows to fail silently. (This supersedes and absorbs **P2-5**.)

1. **You** — full name, business name, **phone (new, PH-1)**, service area.
2. **Services** — which types you offer, each with a default price + duration. Makes contract-fed booking work on the first try, which is F-4's root cause.
3. **Your agreement** — confirm the packaged Pet Services Agreement or acknowledge that counsel review is pending (W-9's standing caveat).

- Skippable per-step like the client flow, but with a persistent "finish setting up" cue until complete.
- ⚠️ Do **not** gate the whole app behind this — a walker who wants to poke around before committing should be able to.

### O-2: Pet owner onboarding — invite, then a read-only portal ✅ *(built 2026-07-18, `95a64d2`)*
Today: walker adds a client → **the owner is told nothing** (P2-13) → they must be verbally told the portal exists → magic link → read-only home with no context.

1. **Invite email on client creation** (P2-13). Uses the existing `emailLayout()` at `NotificationService.ts:78` so branding matches. Welcome, what the portal does, what they signed up for (service, price, cadence — all already on the contract), and a **plain link to `/portal`**. ⚠️ **Not a magic link** — auto-sending one on client creation would burn the ~2/hour Supabase rate limit on people who aren't ready to log in.
2. **Login screen states passwordless up front** (C-6/D1) — "No password needed. Enter your email and we'll send you a secure link." Before the email field, not after.
3. **Portal Profile tab** (C-2, **read-only per D2**) — their details, their pets, and **their walker's contact card** (C-3). Every field carries a "Need to change this? Message your walker" affordance rather than an input.
4. **First-login orientation** — a short "here's what you can do here" panel on first visit only: review and sign agreements, pay invoices, message your walker, see upcoming walks.

#### O-2 message alerting — folded in 2026-07-18 after a targeted investigation

> **A walker's message to an owner triggers nothing at all.** Traced end to end: `MessagingService.sendIntoThread` (`MessagingService.ts:214-254`) inserts the row, touches `last_message_at`, and returns — it never imports `NotificationService`. And there is no template it *could* enqueue: the `NotificationTemplate` union (`NotificationService.ts:20-26`) has no message entry, so **no email, push, or SMS path exists**. The owner also gets no in-app cue: `PortalService.overview()` (`:186-191`) never computes an unread count, so the portal has nothing to badge, and Home surfaces only contracts and invoices. Even the 8-second poll is narrower than it looks — it's created inside `renderThread` (`portal.js:456`) and killed by `stopPolling()` at the top of every `render()` (`portal.js:505-506`), so **it only runs while that one conversation is open on screen**. The professional side has had an unread dot the whole time (`app.js:2085`); owners just never got the equivalent.

**⚠️ Why this is part of O-2 rather than a separate item: it is what makes the D2 read-only decision actually work.** Read-only rests on "message your walker to correct your details." The owner→walker direction is fine — the walker sees an unread dot. The reply direction is dead: the walker answers "updated your number" and the owner has no way to find out without independently opening the portal and clicking into the thread. **Without this, D2's correction loop never closes.**

5. **Unread count in the portal** — `overview()` computes it (the professional-side query at `MessagingService.ts:101-108` is the pattern), Home surfaces it as a cue alongside contracts and invoices, and the Messages nav item carries a badge. **This is the piece that closes the D2 loop** — build it before the message email if only one lands.
6. **A `message_received` notification template** — new entry in the `NotificationTemplate` union plus an enqueue call in `sendIntoThread`, mirroring the existing contract/payment templates. ⚠️ Needs a send rule so a chatty thread doesn't email per message — digest, or "only if the recipient hasn't opened the thread in N minutes." Decide that at pickup.
- **Deliberately not doing yet: owner-side Realtime.** Migration 015 only ever got `message_threads_professional_select` and `messages_professional_select`, both keyed on `professional_account_id`. Its own comment at lines 13-14 describes the owner policy — *"the owner portal adds a second policy here so a client's owner account can read the thread via clients.owner_account_id"* — **which was never written**, so an owner JWT would receive nothing even if the portal subscribed. Polling on Home gets most of the value for a fraction of the work; revisit only if owners turn out to be heavy messagers.

### Build order (T-3 first, per D3)
1. **T-3** — shared markup module, so `fmtPhone` and the profile/contact-card markup have one home.
2. **PH-1** — walker phone field (unblocks C-3, and O-1 step 1 needs it).
3. **O-1** — walker onboarding wizard.
4. **P2-13 + O-2** — owner invite email, then the read-only portal profile, then the unread count (which closes D2's correction loop) and the message email.
5. **PH-2, C-4/P2-7** — vet field split and branded/clickable invoices; both are independent and can slot in anywhere after.

**Founder-side, no code, can happen in parallel:** C-1 (Supabase email template), C-5 (verify the Resend domain **and** flip on Stripe customer receipts).
...
- 2026-07-18 (O-2 + P2-13 built — **C-2 and C-6 closed**): **The owner portal now tells owners things instead of waiting to be checked.** *Unread messages* — `PortalService.overview()` counts messages in the owner's threads they didn't send and haven't read; the portal renders it as a Messages nav badge and a home cue, cleared by opening the thread. **This is the piece that makes D2 work**: read-only rests on "message your walker to correct your details", and until now the walker's reply reached no one. *P2-13 `portal_invite`* — creating a client with an email queues a welcome linking to `/portal`; ⚠️ **deliberately not a magic link**, since Supabase's mailer allows ~2 link emails/hour and auto-sending one on client creation would burn that budget on people who aren't ready to log in. *`message_received`* — the walker's reply emails the owner **5 minutes later**, and the renderer cancels it if they've read the message in-app by then, so a chatty thread doesn't send one email per message (also skipped on duplicate resends). Both templates take `origin` from the request rather than a new env var, matching how the Stripe return and magic-link URLs are already built. *C-2* — read-only Profile tab (details, pets, walker contact card), with "message them and they'll update it" in place of inputs; the walker card moved here from home rather than appearing twice. *C-6* — the passwordless line existed but sat **below** the button, which is exactly why the tester went looking for a signup; it's now above the email field, with the footer carrying the link-expiry hint. Verified: e2e extended (invite queued on client creation; unread asserted as a **delta against a baseline** — an absolute count failed because earlier steps legitimately leave unread rows; deferred send time; cleared after read) — 11/11. Driven in a real portal session: badge "1" + home cue, Profile renders every field with **zero editable inputs**, opening the thread clears both. typecheck clean, no console errors.
  - ✅ **RESOLVED 2026-07-18 — email now sends.** `EMAIL_FROM` on Render is set to `info@eastwestoak.com` (a real monitored mailbox, which matters: `contract_ready` invites clients to reply). Confirmed by a live test send against production — the queue row returned `sent` with no error, where a mismatched sender would have returned a Resend 403 stored as `failed`. **All three new email templates are now testable.** *Original diagnosis kept below, because the failure mode is invisible and will look like a code bug next time.*
  - ⚠️ **BLOCKER for testing the emails — `EMAIL_FROM` on Render points at the wrong domain.** *(corrected 2026-07-18 after the founder checked Render.)* **The verified sending domain is `eastwestoak.com`, not `itchytail.com`** — verified at Resend 2026-07-17, "ready to send and receive". But Render's `EMAIL_FROM` is still `info@itchytail.com`, which is **not** the verified domain, so Resend refuses every send.
    - **The sender is env-only.** `env.ts:40` is the single source (`optional('EMAIL_FROM') ?? 'PetPro Connect <onboarding@resend.dev>'`) and `itchytail` appears nowhere in `src/`, `public/`, or `scripts/`. **No code change can fix this** — it is a Render dashboard edit (this service ignores `render.yaml`).
    - **Fix:** set `EMAIL_FROM` on Render to an address on the verified domain, in `Name <addr>` form — e.g. `PetPro Connect <hello@eastwestoak.com>`. Mirror it in local `.env`, which has no `EMAIL_FROM` at all and so silently falls back to the `onboarding@resend.dev` sandbox sender (owner-inbox only).
    - ⚠️ **Pick an address that can actually receive.** `contract_ready` tells clients *"Questions before then? Just reply to this email"* — if the from-address has no monitored mailbox, those replies vanish. Resend doesn't require the mailbox to exist in order to *send*, so this fails silently.
    - **How to confirm it worked:** `select status, error, payload->>'template' from notification_queue order by created_at desc limit 10;` — `failed` with a Resend domain error means the sender is still wrong; `sent` means it's fixed.
- 2026-07-18 (O-1 built): **Walker onboarding wizard live at `#/setup`** — signup now routes new professionals into three steps (You → Services → Ready) instead of dropping them into an empty app where a contract could generate with a blank business name. **The design decision worth keeping:** "setup complete" is **derived** from the profile (`full_name` + `phone` + ≥1 offered service type), not stored as a flag — no migration, no column to drift out of sync with reality. Today shows a "Finish setting up" cue while that's false; the app is never blocked and every step is skippable. Reuses the existing `.stepper` markup and `PATCH /api/auth/profile` — **no new endpoint, no schema change, no new CSS**, 86 lines total. **Scope held deliberately:** the roadmap's step 2 also asked for a default price + duration per service type; nothing in the schema stores that, and services are contract-born by design (F-4), so it needs a migration and a new concept — logged as **P2-14** instead of being quietly built. Verified in-browser on a fresh signup: signup → `#/setup`, step 1 formatted `5550104477` → `(555)010-4477` on blur and saved business name + phone, step 2 saved two offered types, step 3 read "You're set up", and `/api/auth/me` confirmed all three persisted. Cue absent when complete, returns when offered types are cleared, links to `#/setup`. Skip advances without saving; `#/setup/9`, `/0` and `/abc` all clamp into range. `npm test` 11/11, typecheck clean, no console errors.
- 2026-07-18 (PH-1 + C-3 built): **The walker can finally enter their own phone number, and owners can see it.** `accounts.phone` had existed since migration 001 and sat in `types/index.ts`, but had no UI anywhere — so the number the cold tester asked for had never been enterable. **No migration required.** `PATCH /api/auth/profile` now accepts `phone` and splits it to a new `AccountService.updateAccountPhone`, because phone is an `accounts` column while everything else on that route belongs to `professional_profiles` — duplicating the column would have created two sources of truth. `PortalService.overview()` joins `accounts` for the professional's phone + email, and the portal home renders a **"Your pet care professional"** contact card (read-only per D2: call, email, or message — no editing), de-duplicated by account id since one owner may have several clients with the same walker. **Bug fixed alongside:** `loadProfile()` refreshed only `profile`, so a saved phone re-rendered from a stale cached `account` — it now refreshes both. **Worth knowing for O-1:** `POST /api/auth/signup` has accepted a `phone` field all along (`auth.ts:16`), so onboarding step 1 needs no new backend. Verified: e2e extended with a real round trip (step 8 save + readback, step 11 asserts the portal overview exposes phone and email), and **the new assertion was fault-injected** — removing the save makes step 8 fail, so the coverage is real rather than decorative. Driven in-browser end to end: typed `5550109921`, blur formatted it to `(555)010-9921` via the shared `fmtPhone`, persisted, and re-rendered from the refreshed account. `npm test` 11/11, typecheck clean, no console errors. **Next: O-1** (walker onboarding wizard).
- 2026-07-18 (message alerting investigation): **A walker's message to a pet owner alerts the owner in no way whatsoever — folded into O-2 rather than logged separately, because it is what makes the D2 read-only decision function.** Traced end to end: `sendIntoThread` never imports `NotificationService`, and no message template exists in the `NotificationTemplate` union, so there is no email/push/SMS path *and no way to add one without a new template*. No in-app cue either — `overview()` computes no unread count for owners, Home surfaces only contracts and invoices, and the 8s poll lives inside `renderThread` so it runs **only while that one conversation is open on screen**. Migration 015's own comment (lines 13-14) describes an owner-side RLS policy for Realtime that was never written, so Realtime is not a shortcut here. **The consequence worth remembering:** D2 (read-only portal) assumes owners message their walker to correct details; the owner→walker leg works, but the walker's reply reaches no one, so the loop never closes. The unread count is therefore the higher-priority half — build it before the message email if only one lands. Owner-side Realtime deliberately deferred; polling on Home gets most of the value for a fraction of the work.
- 2026-07-18 (T-3 + urgent copy fix): **`public/shared.js` built — the two frontends now share one copy of everything they had duplicated, and the false receipt promise is gone.** The urgent item first: `portal.js` told users *"A receipt is on its way to your email"* after paying, which depends on the unverified Resend domain and Stripe's customer-email setting — a cold tester was promised an email that never came (C-5). It now points at Payment history on the portal home, which is always true. **T-3 itself:** `esc`, `fmtDate`, `fmtTime`, `fmtMoney`, `fmtPhone`, `installPhoneFormatting`, `withBusy`, `createToast`, and the entire contract sign screen (`contractPane`, `wireContractPane`, `signPadCard`, `createSignaturePad`) moved to a shared module, loaded by both pages as a plain `<script defer>` before their own file — **no build step**, per the existing architecture. **`api()` is deliberately NOT shared:** its 401 branch differs by design (professional re-logs in with a password, owner requests a new magic link), so sharing it would mean parameterising auth for no real gain. **Real drift found while consolidating:** the portal's `withBusy` had lost the `btn.disabled` double-submit guard the professional app has — a fast double-click could fire an action twice. The unified version keeps that guard *and* the portal's null-button handling. **`check-structure.mjs` updated:** the `[data-*]` hooks now live in `shared.js`, so the two frontends **structurally cannot disagree** about the sign screen; the now-moot divergence check for those hooks was replaced with a **load-order check** (shared.js must precede each page's own script, or `window.PetPro` is undefined at startup and the page renders blank — fault-injected both ways and caught). Verified: shared module driven in-browser (zoom in/in/out → 110% with width compensation, `validate()` rejects empty and flags the pad, accepts after a stroke, PNG export confirmed), both pages load with no console errors, `npm test` 11/11 (steps 3 and 11 cover both sign paths), typecheck clean. **Next in the build order: PH-1** (walker phone field), then O-1.
- 2026-07-18 (cold test round 2 — **demo on hold**): **The portal's first genuine cold walkthrough produced six findings (C-1…C-6), and only one is a code defect.** Traced every one to source before triaging. **C-5 is the headline correction:** the missing payment receipt is *not* a Stripe bug and *not* a code bug — `PaymentService.ts:504-515` enqueues both receipt emails correctly on the paid transition; delivery is blocked by (1) the unverified Resend domain, which `npm test` step 10 has been flagging all along, and (2) Stripe test mode not emailing customers by default, which is a Dashboard toggle. **C-3's root cause is upstream of the portal:** `accounts.phone` has existed in the schema and types since migration 001 but has **no UI anywhere**, so the walker has never been able to enter the phone number the owner is asking to see. **C-6 is working as designed** — passwordless owner login is a `CLAUDE.md` hard constraint; the tester went looking for a signup button because nothing told them they didn't need one. **C-2 revealed the portal has no write routes at all**, not just a missing nav link. Three founder decisions locked the same day: **D1** stay passwordless and fix the messaging; **D2** the portal profile is **read-only** (owners message their walker to correct details — no owner write path, since the `clients` row belongs to the professional and an approval story isn't worth Phase 1); **D3** T-3 lands before any owner self-service. Also delivered: a **full phone-format audit** (4 fields consistent, 3 gaps — `accounts.phone` has no UI, `pets.emergency_vet` packs a clinic name and number into one unformattable/unsearchable/un-`tel:`-able string, and the portal has no phone handling) and **two mapped onboarding flows** (O-1 walker wizard, absorbing P2-5; O-2 owner invite + read-only profile, absorbing P2-13). **On tooling:** no external library or starter repo is a fit — the no-build vanilla-JS architecture rules out the React options, and driver.js/Shepherd/intro.js solve product *tours*, not data collection. The reuse that matters is in-house: `renderNewClient`'s existing 3-step `.stepper` (`app.js:605-615`) is the pattern both flows copy, and `emailLayout()` already brands the invite email.
- 2026-07-18 (frontend hardening): **T-1 and T-2 built — `styles.css` is now safe for the design team to rewrite.** Three load-bearing declarations were found by auditing what the JS actually depends on: `[data-doc-shell]`'s height (the F-8 bug), `[data-doc-frame]`'s `transform-origin: 0 0` (zoom scales from centre and drifts without it), and `[data-sigpad]`'s `touch-action: none` + `display: block` (**without `touch-action` a finger drag scrolls the page instead of drawing — signing is impossible on exactly the phones and tablets clients sign on**). These moved into a commented STRUCTURAL CONTRACT block at the top of `styles.css`, keyed on `data-*` attribute selectors so they are visibly not ordinary classes; the cosmetic properties (radius, border, background, cursor) stayed behind in the normal class rules. `scripts/check-structure.mjs` (T-2, no new dependencies) enforces all of it and now gates `npm test`; also runnable alone as `npm run check:structure`. **Verified by fault injection, not just by passing:** deleting the height, neutering it to `auto`, stripping `touch-action`, and removing the hook from `portal.js` only (the exact F-8 divergence shape) were each injected and each caught with a plain-language failure naming what breaks — then the tree was restored and re-verified byte-identical. Post-refactor computed styles confirmed in-browser: shell 420px, frame 419px, `transform-origin: 0px 0px`, `touch-action: none`, cosmetic `border-radius: 10px` still applying. `npm test` 11/11, typecheck clean. **Deliberately still open: T-3** (shared markup module) — the duplication remains, but it can no longer cause a silent regression, which was the actual risk.
- 2026-07-18 (portal regression): **"Testers still don't see the changes" was a real bug, not a caching or deploy problem — and it exposed two bigger things.** Production was verified serving current code (all five static assets byte-identical to `HEAD`, correct `max-age=0`+ETag headers, Cloudflare `DYNAMIC`, no stale second service). The actual cause: F-8 moved the contract pane's height onto a **new `.doc-shell` wrapper** and updated only `app.js`; `portal.js` shares the same `styles.css` but never got the wrapper, so its iframe kept `height: 100%` inside an `align-items: start` grid item with no definite height and collapsed to the replaced-element default. Measured in-browser against the real stylesheet: **150px without the wrapper, 419px with it.** Owners had been reading contracts in a 150px sliver since `d8259b9`. Fixed and deployed (`0330246`): portal iframe wrapped in `.doc-shell`, plus the zoom controls ported from `app.js` for parity (owners sign on phones more than walkers do). `npm test` 11/11, typecheck clean, deploy confirmed live. *Not driven end-to-end on the live portal sign screen — that needs an owner magic-link session plus a contract; the zoom handler is an identical copy of the already-verified professional-side path.*
  - **Bigger thing #1 — the portal has never actually been cold-tested.** The Week 8 task (line 303) said "test the owner portal cold", but every item that came back (F-1…F-5: add-a-pet form, new-invoice form, schedule from client profile, booking a service) lives on screens a pet owner **cannot reach**. The tester was cold, but was sitting in the *professional* app — almost certainly handed an already-logged-in device. F-1…F-9 is good feedback that has never been validated against the portal, and the 150px pane is the proof: nobody had eyes on that screen. **Re-run the cold test properly against `/portal`, with a contract waiting to be signed.**
  - **Bigger thing #2 — the shared-stylesheet coupling that caused this.** Logged as its own section above ("Frontend Architecture"), with a three-tier fix scoped around the incoming UX/UI update.
- 2026-07-18 (review pass): **Code review over the F-1…F-9 diff — two real bugs found and fixed, deployed (`ff8fa37`), production e2e 11/11 after.** (1) The Today agenda filtered `status !== 'canceled'` (one L) against a `'cancelled'` (two Ls) enum — cancelled walks had **never** left the agenda; pre-existing since W-4, caught by reviewing the touched function, fix verified in-browser. (2) The new multi-day self-overlap check ran before the boarding exemption — overlapping multi-day boarding stays would have 409'd despite the "boarding never blocks anything" rule; moved inside the non-boarding branch. Plus two small efficiency cleanups (zoom handler DOM lookups hoisted; search-path color-map fetch parallelized). Reviewed but deliberately not changed: `findConflicts`' linear scan (sub-millisecond at realistic scale) and slimming the color-map's client fetch (needs an API change; revisit if client books get big).
- 2026-07-18 (later): **Second feedback round (F-7…F-9) built same day** — per-client calendar colors (creation-order assignment after the hash approach collided in testing), contract fit-to-window + zoom on the sign screen, signature-hint overlap fixed, and phone/email links restyled from browser blue to ink-with-underline. All UI-only (`public/app.js` + `styles.css`); browser-verified both themes' behavior via computed styles; `npm test` 11/11 after.
- 2026-07-18: **Cold-tester feedback (6 items) triaged and built same day — see the F-1…F-6 section above.** Five build items (collapse add-pet + new-invoice forms, Schedule from the client profile, service pre-filled when booking after a signed contract, multi-day weekly series via `extra_starts`), one question answered (F-6: invoices live in client Billing + the owner portal's payment history; no Stripe account needed; per-invoice printable documents ride with P2-7). F-4's root cause is worth knowing: the booking form still had the Week-6 inline "new service" block — the same drift W-8 removed elsewhere — so the tester was being asked to *create* a service that their signed contract had already created. Verified: `npm test` 11/11 (step 5 now includes a multi-day leg) + `week6-test.ps1` 8/8 locally, full flow driven in-browser (signup → client → pet → contract → sign → Schedule walks → Mon/Wed/Fri series → invoice collapse).

- 2026-07-17 (evening): **Four founder decisions executed in one pass.** (1) **Counsel packet removed** — W-9 closed; the Pet Services Agreement is the only seeded template, the CA v1 file retained unseeded, the new-contract form defaults to it, and week3-test updated accordingly. The Week 3 "have counsel review before a real client signs" recommendation stands as the standing caveat. (2) **P2-12 logged** — professionals uploading their own contracts (the template-conversion work this week is its manual prototype). (3) **Week 5 card leg closed** — founder ran the test-card payment; the weeks 3/5/6 QA re-run is now fully closed. (4) **The Week 6 `[d]` resolved and built**: weekly/biweekly/monthly services now auto-invoice at period end (migration 017 applied live, worker at boot + 5-min interval, exactly-once per period verified by backdating a live weekly service — one invoice, clock advanced, second pass silent). Note for ops: on a sleeping host (Render free tier), invoicing passes only run while the app is awake — one more reason for the paid-tier decision before demo day.
...
| Phase 1.5 — W-5…W-13 | ✅ **Complete and deployed 2026-07-17**, incl. late additions: Pet Services Agreement as the **only** seeded template (counsel packet removed, W-9 closed), and **period-end auto-invoicing for weekly/biweekly/monthly services** (migration 017, resolves the Week 6 `[d]`). QA: e2e 11/11 + week3 + week6 green | ✅ **Week 5 card leg run 2026-07-17.** Remaining: demo prep (see founder list) |
