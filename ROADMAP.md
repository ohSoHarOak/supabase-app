...
- [ ] **C-6: no signup / password creation for owners.** **Working as designed** — `CLAUDE.md` locks *magic-link for owners, email/password for professionals*, and the owner account is auto-created on first link click (`PortalService.ts:101`). **Decision locked (D1): stay passwordless; fix the messaging.** The tester went looking for a signup button because nothing told them they didn't need one. The login screen must say so before the email field — passwordless is the right call for someone who logs in a handful of times a year, but only if it's stated.
...
### O-2: Pet owner onboarding — invite, then a read-only portal
